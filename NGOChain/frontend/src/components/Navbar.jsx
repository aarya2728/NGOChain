...
function Navbar() {
  ...
  useEffect(() => {
    ...
    if (provider) {
      // Only auto-fetch accounts if the user previously connected manually
    ...
    <nav className="bg-gray-900 text-white shadow-md px-6 py-4 flex justify-between items-center sticky top-0 z-50 border-b border-gray-800">
      ...
      <div className="flex gap-3 items-center">
        ...
        {!walletAddress && (
          <Link 
            to="/login"
            className="px-5 py-2 rounded-full font-bold bg-teal-500 text-gray-900 hover:bg-teal-400 transition-colors shadow-lg shadow-teal-500/20 text-sm"
          >
            Login
          </Link>
        )}
